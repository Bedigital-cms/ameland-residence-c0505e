...
export default async function HomePage({ params }: { params: Promise<{ locale: string }> }) {
   ...
   * Ameland" / its German equivalent) but rendered it as a <p>, leaving the homepage without an h1.
   * Promote that existing text to the <h1>; no content changes.
   *
   * `home.json` has a SECOND hero further down (section 8). `pageHeading` returns the first hero's
   * index, so only that one becomes the h1 — the lower hero keeps its <p> and the page has exactly one.
  ...
  return (
    <Shell locale={locale}>
      <JsonLd json={jsonld} />
      <Sections sections={sections} ctx={buildCtx(locale)} opts={opts} />
    </Shell>
  )
}
